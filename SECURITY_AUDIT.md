...
1. **エラー情報の露出** ✅ 修正済み
   - 500エラー時に `details: errorMessage` で内部エラーを返していた
   - クライアントには汎用メッセージのみ返すように変更

2. **入力サイズ制限（translate-english）** ✅ 追加済み
   - テキスト: 10,000文字以内
   - 画像 base64: 10MB 以内

### 推奨事項（未実装）

- **レート制限**: APIの濫用・コスト爆発を防ぐため、Vercelやミドルウェアでのレート制限を検討
- **認証**: 外部公開時はAPIキーや認証の導入を検討
